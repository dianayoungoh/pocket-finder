    ...
    <div className="legend">
      ...
      {pockets.map((pocket, index) => (
        <div key={index} className="legend-item">
          <h4>Pocket {index + 1} ({pocket.length} faces)</h4>
          <ul>
            {pocket.map((entityId) => {
              ...
              return (
                <li key={entityId}>
                  ...
                  📍 <strong>Center:</strong> {info?.centerPoint?.map(coord => coord.toFixed(2)).join(", ") || "N/A"} | 
                  📏 <strong>Area:</strong> {info?.area?.toFixed(2) ?? "N/A"} mm²
                </li>
              );
            })}
          </ul>
